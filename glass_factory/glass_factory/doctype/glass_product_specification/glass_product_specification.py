...
class GlassProductSpecification(Document):
	...
	def update_item_code_preview(self):
		if not self.glass_type or flt(self.thickness_mm) <= 0 or flt(self.length_mm) <= 0 or flt(self.width_mm) <= 0:
	...
	def refresh_preview(self):
		...
		result = {
			...
			"raw_sheet_width_mm": self.raw_sheet_width_mm,
			"raw_sheet_area_m2": self.raw_sheet_area_m2,
			"item_code_preview": self.item_code_preview,
	...
	def generate_items(self):
		...
		if cint(self.items_generated):
			if self.generation_status != "Regeneration Required" and not cint(self.allow_regeneration):
				frappe.throw(
